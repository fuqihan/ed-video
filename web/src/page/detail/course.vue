<template>
  <div class="detail-course" v-show="detailCourseLoading">
    ...
    <div class="detail-course-footer">
      ...
      <a href='#/video/123' class='button'>学习</a>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      api.courses.findCoursesById(base64.decode(this.$route.params.id)).then(data => {
        htmlBody.scrollTop = 0
        ...
        setTimeout(() => {
          ...
          let titleScrollTop
          /* 课程列表距离顶部的距离 */
          let tableHeight = courseNavHeight + this.$refs.detailCourseTable.offsetHeight + this.$refs.detailCourseTeacher.offsetHeight + 2
          let teacherHeight = courseNavHeight + this.$refs.detailCourseTeacher.offsetHeight
          this.courseTop.table = this.$refs.detailCourseTable.offsetTop - courseNavHeight * 2
          this.courseTop.teacher = this.$refs.detailCourseTeacher.offsetTop - courseNavHeight * 2
          this.courseTop.review = this.$refs.detailCourseReview.offsetTop - courseNavHeight * 2
          /*  -----------  */
          this.setTime = () => {
            /*  使courseNav随着滑动能顶到最上面  */
            titleTop = this.$refs.courseTitle.getBoundingClientRect().top
            titleScrollTop = -1
            titleTop.toString().split('-').length === 2 ? titleScrollTop = titleTop.toString().split('-')[1] : 0
            if (this.$refs.courseNav.getBoundingClientRect().top <= 0 && titleScrollTop >= this.$refs.courseTitle.offsetHeight) {
              this.courseNavStyle.off = true
              tableHeight = courseNavHeight + this.$refs.detailCourseTable.offsetHeight + this.$refs.detailCourseTeacher.offsetHeight
            } else {
              this.courseNavStyle.off = false
              tableHeight = this.$refs.detailCourseTable.offsetHeight + this.$refs.detailCourseTeacher.offsetHeight
            }
            if (teacherHeight < this.$refs.detailCourseReview.getBoundingClientRect().top &&
              this.$refs.detailCourseReview.getBoundingClientRect().top <= tableHeight &&
              this.courseNavItemStyle.item !== 2) {
              this.courseNavItemStyle.item = 2
            } else if (courseNavHeight < this.$refs.detailCourseReview.getBoundingClientRect().top &&
              this.$refs.detailCourseReview.getBoundingClientRect().top <= teacherHeight &&
              this.courseNavItemStyle.item !== 3) {
              this.courseNavItemStyle.item = 3
            } else if (this.$refs.detailCourseReview.getBoundingClientRect().top <= courseNavHeight &&
              this.courseNavItemStyle.item !== 4) {
              this.courseNavItemStyle.item = 4
            } else if (this.$refs.detailCourseReview.getBoundingClientRect().top > tableHeight &&
  ...
  .detail-course {
    ...
    .detail-course-footer {
      ...
      .button {
        ...
        background: #3e779d;
        background: -webkit-gradient(linear, left top, left bottom, from(#6637cc), to(#3e779d));
      ...
      .button:hover {
        ...
        background: #3e779d;
        background: -webkit-gradient(linear, left top, left bottom, from(#4e338a), to(#3e779d));
