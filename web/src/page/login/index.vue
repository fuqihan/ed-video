...
export default {
  ...
  created() {
    let name = this.$store.state.loginBefore
      ? this.$store.state.loginBefore
      : window.localStorage.getItem('edVideoLoginBefore')
        ? window.localStorage.getItem('edVideoLoginBefore')
        : "index";
  ...
  methods: {
    ...
    onWeibo() {
      this.outhHtml = window.open(
        `https://api.weibo.com/oauth2/authorize?client_id=456805882&response_type=code&state=123&redirect_uri=http://www.fuqihan.cn/node/login/getWeiboAccessToken`
      );
      this.show = true;
    },
    onBaidu() {
