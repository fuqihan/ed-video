<template>
  <div ref="statisticsChart" style="width: 49%;height:400px;padding-top: 20px"></div>
</template>
    ...
    mounted() {
      const option = {
        ...
        yAxis: [{
          ...
          splitLine: {
            lineStyle: {
              color: '#dddddd' //分隔线颜色设置
            }
          }
        }],
        series: [{
          name: '前天', //系列名称，用于tooltip的显示，legend 的图例筛选，在 setOption 更新数据和配置项时用于指定对应的系列
          type: 'line',
          ...
          areaStyle: { //区域填充样式
            normal: {
              color: this.$echarts.graphic.LinearGradient(0, 0, 0, 1, [{ //填充的颜色。
                offset: 0, // 0% 处的颜色
      ...
      let myChart = this.$echarts.init(this.$refs.statisticsChart);
